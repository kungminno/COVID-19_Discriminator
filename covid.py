...
def feature_extractor_rec(row):
    ...
    try:
        ...
        mfccsscaled = np.mean(mfccs.T, axis=0)  # 20
        ...
        contrast = librosa.feature.spectral_contrast(y=audio, sr=sr)
        contrastscaled = np.mean(contrast.T, axis=0)  # 7

        flatness = librosa.feature.spectral_flatness(y=audio)  # 1
        flatnessscaled = np.mean(flatness.T, axis=0)

        Feature_Set_rec = np.concatenate([mfccsscaled, chroma_vecscaled, bandwidthscaled, contrastscaled, flatnessscaled])
        '''
        ...
        delta2scaled = np.mean(mfcc_delta2.T, axis=0)  # 20

        # 열을 따라 산술 평균을 계산



        Feature_Set_rec = np.concatenate((mfccsscaled, delta2scaled),axis=None)
...
def mod(cough_segments, addr):
    MERGM_f = load_model('C:/Users/kungm/Desktop/test/M_5322_md2_40_e4_2.h5')
...
def recv_data(client_sock, addr, client_sockets):
    ...
    while 1:
        ...
        if n == 20:
            ...
            preds = [pred for pred in y_preds_test if pred > 0.5]
            if len(preds)>=1:
                p_p = sum(preds) / len(preds) *100
...
def main():
    ...
    host = '192.168.0.221'#'172.30.1.35'#'192.168.75.121'#'172.17.97.211'#'172.30.1.22'   #home com:'172.30.1.16', lab: '117.16.123.50'
    port = 9999
